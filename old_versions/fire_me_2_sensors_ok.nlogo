...
if any? sensors with [count embers with [color = 45] in-radius 20 > 0 or count embers with [color = 15] in-radius 20 > 0]   [
 ...
 ask  sensors [
if not any? sensors with [count embers with [color = 45] in-radius 20 > 0 or count embers with [color = 15] in-radius 20 > 0] [
...
to process-airplane-queue
  ...
  if get-performative msg = "request" and get-content msg = "fire-stopped" [
   ask airplanes [set status "return-to-base"]
  ]
...
to rest
  ifelse tod > 180  [
    ask airplanes [ face one-of patches with [pcolor = red]
        fd 5]
  ]
...
657
show_range
ask sensors [ask patches in-radius  120 WITH \n[ DISTANCE MYSELF > 119 ] \n[set pcolor 25]\n]\n
NIL
1
T
OBSERVER
