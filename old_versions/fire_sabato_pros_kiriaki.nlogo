...
if any? sensors with [count embers with [color = 45] in-radius 150 > 0  or count embers with [color = 15] in-radius 150 > 0]   [
 ...
 ask  sensors [
if not any? sensors with [count embers with [color = 45] in-radius 150 > 0  or count embers with [color = 15] in-radius 150 > 0] [
...
to ask_airplane_to_back_sensor
    send add-receiver my-plane add-content "fire-stopped" create-message "inform"
...
to do_run_airplane
    ...
    ask airplanes [
       ...
       set heading towards one-of embers with [color = 45 or color = 15 or color = 133]
       fd 5
        set result   0
   ...
   if status = "take-off" and count embers  with [color = 45] = 0  and count embers with [color = 15] = 0
    [return_to_base]
...
to return_to_base
     ...
     ;face one-of patches with [pcolor = 120];γυρνα προς την βάση και επέστρεψε σε αυτή
       ...
       ;fd 5
       if any? patches with[ pcolor = 120]
[set heading towards one-of patches with[ pcolor = 120 ] fd 5]
       ]
...
to process-airplane-queue
  ...
  if get-performative msg = "request" and get-content msg = "fire!fire!fire!"  [
    if status = "stand-by" [
      ask airplanes [set status "take-off"]
    ]]
