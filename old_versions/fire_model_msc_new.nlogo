...
airplanes-own [alert incoming-queue status];ουρές μηνυμάτων αεροσκαφών και ποσοτητα νερου που φέρουν πανω τους
sensors-own [alert incoming-queue status];ουρές μηνυμάτων σενσορς
...
to do_run_sensor
if count embers with [color = 45] > 0 or count embers with [color = 15] > 0 [
  set status "firing"
  ask_airplane_to_send
 ]
if count embers with [color = 45] = 0 or count embers with [color = 15] = 0
[ set status "stop-fire"
  ask_airplane_to_back
]
...
;*****************************************
;*****************************************
;Κίνηση αεροσκαφών προς την φωτιά
to move
