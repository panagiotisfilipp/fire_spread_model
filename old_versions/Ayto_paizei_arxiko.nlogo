...
if any? sensors with [any? embers with [color = 45] in-radius 30  or any? embers with [color = 15] in-radius 30]   [
 ...
 ask  sensors [
if not any? sensors with [any? embers with [color = 45] in-radius 30  or any? embers with [color = 15] in-radius 30] [
...
to do_run_airplane
 ...
 [ if status = "take-off"
    [ move-to-fire
    ; if not any? embers with [color = 45] or any? embers with [color = 15]
...
to move-to-fire
  ;ask airplanes [set label "ok"]
  ;face one-of embers
  ifelse any? embers with [color = 45] or any? embers with [color = 15]
...
to move-to-base
  if not any? embers with [color = 45] or not any? embers with [color = 15][
   face one-of patches with [pcolor = 120]
   fd 5
...
to fade-embers
  ask embers
    [ set  burnttime burnttime + 1
      ...
      if burnttime > 20 [set pcolor 1 die]]
...
657
show_range
ask sensors [ask patches in-radius  60 WITH \n[ DISTANCE MYSELF > 59 ] \n[set pcolor 125]\n]\n
NIL
1
T
OBSERVER
