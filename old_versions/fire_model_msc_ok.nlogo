...
sensors-own [incoming-queue status];ουρές μηνυμάτων σενσορς
...
ask sensors [
ifelse count embers with [color = 45] in-radius 20 > 0 or count embers with [color = 15] in-radius 20 > 0 [
  set status "firing"

  ask_airplane_to_send
 ]

[
 set status "scanning"

 if status != "firing" [ask_airplane_to_back]
]]
...
to process-airplane-queue
  ...
  ; Αν το μήνυμα είναι "request" με content "bring beer"
  ; και δεν είμαι απασχολημένος, τότε ξεκινώ να φέρω μπύρα
  let msg get-message
...
to rest
  ifelse tod > 180  [
    ask airplanes [ face one-of airbases
        fd 5]
  ]
  [set status "return to base"]
end
...
to go
   ...
   ; ask airplanes [update-airplanes
