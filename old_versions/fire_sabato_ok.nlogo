...
airplanes-own [incoming-queue status];ουρές μηνυμάτων αεροσκαφών και ποσοτητα νερου που φέρουν πανω τους
...
to setup-sensors
   ...
   create-sensors 1 [
     ...
     set status "scanning"
     set state 0
      ;ask patches in-radius  225  WITH [ DISTANCE MYSELF > 224 ] [set pcolor 25]
...
if any? sensors with [count embers with [color = 45] in-radius 50 > 0 or count embers with [color = 15] in-radius 50 > 0]   [
  ask sensors [set state 1]
  ;ask sensor 0  [set state 1]
     ;ask sensor 1  [set state 1]
     ; ask sensor 2  [set state 1]
      ; ask sensor 3  [set state 1]

  decide_to_do
 ]
]
 ask  sensors [
if not any? sensors with [count embers with [color = 45] in-radius 50 > 0 or count embers with [color = 15] in-radius 50 > 0] [
    ask sensors [set state 2]
    ;ask sensor 0  [set state 2]
     ;ask sensor 1  [set state 2]
     ; ask sensor 2  [set state 2]
      ; ask sensor 3  [set state 2]
   decide_to_do
   ]
]
end
...
to do_run_airplane
  ...
  if status = "take-off" and count embers with [color = 45] > 0 or count embers with [color = 15] > 0
    [
    ask airplanes [
      process-airplane-queue
       set heading towards one-of embers with [color = 45 or color = 15]
       fd 5
        set result   0
    ...
    [process-airplane-queue
     ...
     face one-of patches with [pcolor = 120];γυρνα προς την βάση και επέστρεψε σε αυτή

       fd 5]]
    if   any? airplanes-on patch 0 0 and status = "return-to-base" [
        set result 1
        ]
...
to process-airplane-queue
  ...
  if get-performative msg = "request" and get-content msg = "fire-stopped" [
    if status = "take-off"[
...
;κινηση αφων αν ειναι νυχτα η μέρα
to rest
  ifelse tod > 180  [
    ask airplanes [ face one-of patches with [pcolor = 120]
        fd 5]
  ]
  [set status "return-to-base"]
end
...
to update-airplanes
 ...
 ;water-supply
  ask embers with [color = 45 or color = 15][
    if (count airplanes-on  neighbors) >= 2 [
    set pcolor 1
...
to fade-embers
  ask embers
    [ set  burnttime burnttime + 1
      ...
      ;if burnttime < 10 [set color yellow]
      ;]
end
...
657
show_range
ask sensors [ask patches in-radius  50 WITH \n[ DISTANCE MYSELF > 49 ] \n[set pcolor 125]\n]\n
NIL
1
T
OBSERVER
...
false
0
Circle -955883 true false 105 80 90
Rectangle -955883 true false 90 255 210 300
Line -16777216 false 75 255 225 255
Polygon -955883 true false 90 255 120 150 180 150 210 255
